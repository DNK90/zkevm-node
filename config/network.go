...
func (cfg *Config) loadNetworkConfig(ctx *cli.Context) {
	...
	case string(ronin):
		networkJSON = SaigonRoninNetworkConfigJSON
	case string(custom):
		var err error
	...
	default:
		log.Fatalf("unsupported --network value. Must be one of: [%s, %s, %s, %s, %s]", mainnet, testnet, cardona, ronin, custom)
	}
